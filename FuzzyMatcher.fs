...
module FuzzyMatcher =
    let inline private existsWin mChar str offset radius =
        ...
        if endPos - pos < 0 then 
            ...
            let rec exists index =
                if str.[index] = mChar then 
                    true
                elif index = endPos then 
                    false
                else 
                    exists (index + 1)
    ...
    let inline private jaro s1 s2 =
        ...
        let common (s1 : string, s2 : string) =
            let rec inner i result =
                ...
                | -1 -> result
                | _ ->
                    if existsWin s1.[i] s2 i matchRadius then 
                        inner (i - 1) (s1.[i] :: result)
                    else 
                        inner (i - 1) result
        ...
        let transpos =
            let rec inner cl1 cl2 result =
                ...
                | c1h :: c1t, c2h :: c2t ->
                    if c1h <> c2h then 
                        inner c1t c2t (result + 1.0)
                    else 
                        inner c1t c2t result
    ...
    let dist (s1, s2) =
        if String.length s1 = 0 || String.length  s2 = 0 then 
            ...
            let score =
                ...
                let rec calcLength idx acc =
                    if idx > minLength || s1.[idx] <> s2.[idx] then 
                        acc
                    else 
                        calcLength (idx + 1) (acc + 1.0)
    ...
    let inline private internalMatch words input =
        words
        |> Seq.filter(fun word -> abs (String.length word - String.length input) <= 3)
        |> Seq.map (fun word -> (word, dist (word, input)))
        |> Seq.sortByDescending snd

    /// Finds the best match for a given word
    [<CompiledNameAttribute("Match")>]
    let fuzzyMatch words input =
        internalMatch words input
        |> Seq.head
        |> fst